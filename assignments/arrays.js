...
let carModels = inventory.sort(function(item1, item2){
    if(item1.car_model > item2.car_model){
        return 1
    }else if(item1.car_model < item2.car_model){
        return -1
    }
})
for(let i of carModels){
    console.log(i.car_model)
}
...
let BMWAndAudi = inventory.filter(getCars)
function getCars(item){
